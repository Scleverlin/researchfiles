...
open(DATA, ">./Hybrid_${bit}_BK${depthofbk}_SA${depthofsa}.v") or die "BK${depthofbk}_SA${depthofsa}.v 文件无法打开, $!";
$max_of_pg=$bit-1;
open(DATA4, ">>./_mul_names.txt");
print DATA4"
perl /home/shi/research/adder_project/mul_flex_gen.pl -w Hybrid_${bit}_BK${depthofbk}_SA${depthofsa} ${bit} \n
";
close(DATA4);
print DATA "
...
if ($depthofbk==0){
 ...
 //assign pp_level1[0]=p[0];
        generate
            for (i = 1;i<${for_index} ;i=i+2 ) begin
 ...
 wire [${max_of_pg}:0] pp_level${i};  
     ...
     for (my $k=0;$k<$starting_point;$k++){
        print ("work!\n");
           ...
           //11111
         generate
             for (i = ${starting_point};i<${for_index} ;i=i+${add_index}) begin
...
if ($depthofbk==0){
   print DATA" // ${depthofsa} is deep enough , no post process"
}
else {
